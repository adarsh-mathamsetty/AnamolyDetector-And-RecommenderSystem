...
for i=1:num_movies
numofusersvoted = find(R(i, :) == 1);
temptheta = Theta(numofusersvoted, :);
tempY = Y(i, numofusersvoted);
X_grad(i, :) = (X(i, :) * temptheta' - tempY) * temptheta + lambda * X(i, :);
                end
                                          
for i=1:num_users
numofmoviesvoted = find(R(:, i) == 1);
tempX = X(numofmoviesvoted, :);
tempY = Y(numofmoviesvoted, i);
